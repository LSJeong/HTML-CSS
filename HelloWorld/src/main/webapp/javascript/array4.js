...
//filter : -> map(id,fn,ln,em) -> forEach
//여회원리스트
//표 형식

const female = data.filter(function (val, ind) {
    return val.gender == "Female";
});

console.log(female);

const list = female.map(function (val, ind, ary) {
    let newObj = {}
    newObj.id = val.id;
    newObj.fn = val.first_name;
    newObj.ln = val.last_name;
    newObj.em = val.email;
...
str = '<h3 style="margin-left: 120px;"> &lt; 여회원리스트 &gt; </h3>'
str += '<table border="1">';
str += '<thead style="background-color: gray; color: white;"><th>ID</th><th>성</th><th>이름</th><th>이메일</th>'
str += '<tbody>';
list.forEach(callBackFnc);
str += '</tbody>';
str += '</table>';
document.write(str);

function callBackFnc(val, ind) {
    str += '<tr><td width="40px">' + val.id + '</td><td width="80px">' + val.fn + '</td><td width="80px">' + val.ln + '</td><td>' + val.em + '</td></tr>';
};
